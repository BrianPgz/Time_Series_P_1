...
Como los intervalos de los coeficientes no intersectan a cero, son significativos. 
...
Con un $p-value=0.1997$, obtenemos normalidad de residuales.


```{r echo=TRUE}
shapiro.test(Model1$residuals)
```
Con un $p-value=0.1924$, obtenemos normalidad de residuales con ambas pruebas. 
...
```
Con un $p-value=0.928$ no rechazamos hipótesis nula, entonces tiene media cero. 
...
Observamos que en varios lag se salen de las bandas de coinfianza, vemos que están muy correlacionados los residuales.

Con el $Model1$ el problema que tenemos es la varianza no constante y la alta correlación de los residuales.
...
confint(Model2)
```
Tenemos coeficientes significativos. 
