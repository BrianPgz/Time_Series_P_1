...
Vamos a hacer una transformación para tratar de obtener varianza constante. 
...
Con un $p-value=0.03871$ menor al $.05$, rechazamos $Ho$ entonces la serie es estacionaria. 
...
Veamos qué pasa cuando aplicamos una diferencia a los datos originales. 

```{r echo=TRUE}
diff_S_berr = diff(S_beer)
plot(diff_S_berr)   

```

Con este método quitamos la tendencia, se observan ciclos pero tenemos varianza creciente
